...
- Q: What happens when the athlete has an active Goal but no PlannedWorkout row exists for today (plan gap between blocks, plan ran out, generation bug)? → A: The card resolves to `REST_DAY`. A day with an active Goal but no PlannedWorkout is treated as a recovery day with tips, not an alarm state. Distinct from `NO_PLAN` (no active Goal at all). Silently treating plan gaps as rest days keeps the athlete moving rather than surfacing internal plan-coverage errors as an athlete-facing problem.
...
### User Story 6 — Any Day Without a Scheduled Workout Shows as a Rest Day (Priority: P3)

An athlete has an active Goal and a training plan, but today has no PlannedWorkout row — they're between training blocks, the plan ran out before the race date, or a plan-generation bug left a hole. They open the dashboard. The Today Card displays `REST_DAY` state with the headline "Recovery is the workout", the same health-stats cover lines as a deliberate rest day, and a rationale from the morning check-in or the rule-based fallback. The card shows concrete recovery tips (Sleep / Fuel / Move cues) as part of the REST_DAY cues strip.

**Why this priority**: Treating all no-planned-row days as REST_DAY keeps the athlete moving and avoids surfacing internal plan-coverage failures as an athlete-facing alarm. A blank recovery day is always better than an "attention needed" error card.

**Independent Test**: Seed an athlete with an active Goal but zero PlannedWorkout rows for today (and no `rest`-type row). Hit `GET /api/today` — verify `state = "REST_DAY"`, headline is "Recovery is the workout", `cover_lines` has 4 health-stat entries, and `cues` contains 3 entries with labels Sleep, Fuel, and Move. Verify `actions.cta` is null. (See `test_no_planned_row_treated_as_rest_day` — the FR-033 regression scenario for this change.)

**Acceptance Scenarios**:

1. **Given** an athlete with an active Goal but no PlannedWorkout for today, **When** the card loads, **Then** state is `REST_DAY`, headline reads "Recovery is the workout", and the cues strip shows Sleep / Fuel / Move tips.
2. **Given** the athlete taps the rationale paragraph, **When** the chat panel opens, **Then** the input is pre-filled with "How should I make the most of today's recovery?" — the same prompt as a deliberate rest day.
3. **Given** REST_DAY state (no planned row), **When** the cover lines render, **Then** they show the four morning health stats from HealthSnapshot — same as a deliberate rest day, no CTA button.
4. **Given** the athlete records a CompletedWorkout for today (impromptu run on a no-planned-row day), **When** the activity poll picks it up, **Then** the card transitions to `COMPLETED` with `is_bonus = true` rather than staying in `REST_DAY`.
...
- **FR-012**: The `cover_lines` array MUST always contain exactly 4 entries in PRE_RUN, REST_DAY, COMPLETED, and RACE_DAY states. `NO_PLAN` MAY omit cover_lines (the CTA button replaces them). Each entry MUST have fields `label`, `value`, and (for non-RACE_DAY states) `drill_to` pointing to either `"morning"` (Morning Readiness section) or `"last_run"` (Last Run section). REST_DAY payloads (including no-planned-row days) MUST also include a `cues` array of 3 dicts with `label` and `copy` keys (hardcoded to Sleep / Fuel / Move in v1).
...
- **FR-022**: Tapping the workout headline (in PRE_RUN, COMPLETED, REST_DAY, RACE_DAY states) or the primary CTA button (in NO_PLAN state) MUST open the existing chat panel with the input pre-filled. State-specific prompts: PRE_RUN→"Tell me about today's workout.", COMPLETED→"How did today's run go?", REST_DAY→"How should I make the most of today's recovery?", RACE_DAY→"Walk me through race execution.", NO_PLAN→"I want to train for…". The NO_PLAN state's CTA button label is "Pick a race". In NO_PLAN state the CTA button is the sole tap target — there is no workout headline.
...
- **FR-033**: A new integration test file `tests/integration/web/test_today_api.py` MUST cover, at minimum, one acceptance scenario per user story (across all five user stories), the rationale-source fallback ladder for each state (3–4 scenarios), the four cover-line sourcing branches, and one regression scenario (`test_no_planned_row_treated_as_rest_day`) asserting that an athlete with an active Goal but no PlannedWorkout row returns `REST_DAY` (not `NO_PLAN`), with headline "Recovery is the workout", 4 cover_lines, and a `cues` array of length 3 with labels Sleep / Fuel / Move. (This is the regression for the five-state model; see `docs/superpowers/specs/2026-05-19-today-card-rest-day-design.md`.)
