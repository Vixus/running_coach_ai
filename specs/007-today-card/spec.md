...
- **FR-008**: A new module `coach/today_rationale.py` MUST provide a pure function `rule_based_morning(snap: HealthSnapshot | None, athlete_name: str) -> str` that returns a single short paragraph based on HRV vs baseline, sleep duration, and body battery. The function MUST handle `snap=None` by returning a generic-but-personalized line ("I don't have today's data yet — let's run the plan as written and check in tomorrow.").
...
- **FR-012**: The `cover_lines` array MUST always contain exactly 4 entries in PRE_RUN, REST_DAY, COMPLETED, and RACE_DAY states (`NO_PLAN` MAY omit cover_lines). Each entry MUST have fields `label`, `value`, and (for non-RACE_DAY states) `drill_to` pointing to either `"morning"` (Morning Readiness section) or `"last_run"` (Last Run section).
...
- **FR-022**: Tapping the workout headline (in PRE_RUN, COMPLETED, REST_DAY, RACE_DAY states) or the "Pick a race" primary button (in NO_PLAN state) MUST open the existing chat panel with the input pre-filled. State-specific prompts: PRE_RUN→"Tell me about today's workout.", COMPLETED→"How did today's run go?", REST_DAY→"How should I make the most of today's recovery?", RACE_DAY→"Walk me through race execution.", NO_PLAN→"I want to train for…". The NO_PLAN state has no workout headline — the "Pick a race" button is the sole tap target.
...
- **FR-026**: The frontend MUST poll `GET /api/today` every 60 seconds while the page tab is visible (using the existing `visibilitychange` pattern), and additionally trigger an immediate re-fetch when the notification bell registers a new notification of kind `morning_checkin` or `post_run_feedback`.
...
- **FR-033**: A new integration test file `tests/integration/web/test_today_api.py` MUST cover, at minimum, one acceptance scenario per user story (5 scenarios × 5 user stories = 25 scenarios), the rationale-source fallback ladder for each state (3–4 scenarios), and the four cover-line sourcing branches.
- **FR-034**: A new unit test file `tests/unit/test_today_rationale.py` MUST cover the rule-based fallback templates against representative `HealthSnapshot` permutations (good HRV, poor HRV, missing sleep, missing snap entirely) and the paragraph-extraction helper against representative morning_checkin and coach_analysis bodies.
...
These were intentionally deferred from the brainstorming session and should be answered before the plan phase:

- Q: How exactly does the morning_checkin Claude prompt today produce its body? Is there a single template in `coach/prompt.py` that we update, or is the body assembled from multiple Claude calls? (Implementation detail — affects how FR-009 is realized.)
- Q: Does the existing chat panel have a tap-outside-to-dismiss handler? If yes, does it work with pre-filled inputs? (Affects FR-025 — whether we add the handler or just verify it.)
- Q: For the rule-based fallback (FR-008) — what are the exact template strings for each HRV / sleep / body battery permutation? (Affects unit test coverage.)
- Q: Are there any plan structures where today has *no* PlannedWorkout row (rather than a `rest`-type row)? If yes, how do we distinguish "rest day" from "athlete is between blocks but technically still has an active Goal"? (Affects state-resolution precedence in FR-003.)
