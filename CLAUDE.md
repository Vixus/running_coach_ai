...
The dashboard's top section is a magazine-cover Today Card driven by `GET /api/today`. Five states (`PRE_RUN`, `COMPLETED`, `REST_DAY`, `RACE_DAY`, `NO_PLAN`) per FR-003 precedence. REST_DAY now also covers the "no PlannedWorkout row for today" case (previously a separate OFF_PLAN state). Rationale source ladder per state: `morning_checkin` notification body → `coach_analysis` (COMPLETED) → rule-based fallback in `coach/today_rationale.py` (two branches: with-snapshot ~6 templates, no-snapshot run-by-feel) → `placeholder` (PRE_RUN/REST_DAY before 7am local) → `persona_static` (RACE_DAY/NO_PLAN). REST_DAY uses `coach/today_rationale.py:rule_based_rest` for its rule-based fallback (not `rule_based_morning`), so the prose is rest-appropriate — no "run it" framing on a recovery day. The morning_checkin and post-run prompts in `coach/adapter.py` and `coach/feedback.py` are constrained to emit an opening athlete-centered single-paragraph rationale that the Today Card extracts via `extract_rationale_paragraph()`. State transitions emit a `category="today"` WebEvent only when the state changes (compared against the prior such event for the athlete) — admin Events filter dropdown has `today` as a category.
...
| `web/api/today.py` | `GET /api/today` — Today Card payload (five states: PRE_RUN, COMPLETED, REST_DAY, RACE_DAY, NO_PLAN); emits `category="today"` WebEvent only on state transitions via an `@after_request` hook. Athlete-local timezone, zero Claude calls at request time. |
